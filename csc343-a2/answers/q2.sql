...
SELECT name artists, avg avg_collab_sales
FROM Artist, Album, ColSales
WHERE Album.artist_id = Artist.artist_id
  AND ColSales.artist_id = Artist.artist_id
  AND Album.album_id NOT IN (SELECT album_id FROM ColAlbum)
  AND Album.sales < (
      SELECT avg
      FROM ColSales
